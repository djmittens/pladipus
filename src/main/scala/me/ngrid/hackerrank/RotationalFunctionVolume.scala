...
/**
  */
object RotationalFunctionVolume {
  ...
  def area(coefficients:List[Int],powers:List[Int],x:Double):Double =
  {
    ...
    // For the given coefficients, powers and value of x
    val r = coefficients.zip(powers).
      map({case (c, p) => c * Math.pow(x, p)}).sum
  ...
  def summation(func:(List[Int],List[Int],Double)=>Double,upperLimit:Int,lowerLimit:Int,coefficients:List[Int],powers:List[Int]):Double =
  {
    ...
    lowerLimit.toDouble.to(upperLimit, 0.001).map(func(coefficients, powers, _)).sum
  }
